...
<head>
<meta charset="UTF-8" />
<title><spring:message code="title.example.crud.emp.list" /> <system:properties key="servlet.container.id" /></title>
<%@ include file="/WEB-INF/jspf/script.jspf"%>
<%@ include file="/WEB-INF/jspf/style.jspf"%>
<%@ include file="/WEB-INF/jspf/example/favicon.jspf"%>
<script type="text/javascript" src="${pageContext.request.contextPath}/js/example/crud/emp/emp.js"></script>
</head>
<body class="composite">
	...
	<div id="bodyColumn">
		<div id="contentBox">
			<div class="section">
				...
				<c:url var="listEmpUrl" value="/example/crud/emp/listEmp" />
				...
				<!-- import / export area -->
				<c:url var="importEmpListUrl" value="/example/crud/emp/importEmpList2" />
							...
							<tr class="b">
								...
								<td><c:choose>
										<c:when test="${not empty pageContext.request.queryString}">
											<a href="${pageContext.request.contextPath}/example/crud/emp/exportEmpListOnCurrentPage?<c:out value="${pageContext.request.queryString}" />"> <spring:message
													code="button.export.current.page" />
											</a>
										</c:when>
										<c:otherwise>
											<a href="${pageContext.request.contextPath}/example/crud/emp/exportEmpListOnCurrentPage"> <spring:message code="button.export.current.page" />
											</a>
										</c:otherwise>
									</c:choose> &nbsp; <%
 ...
 %> <c:choose>
										<c:when test="${not empty replacedQueryString}">
											<a href="${pageContext.request.contextPath}/example/crud/emp/exportEmpList?<c:out value="${replacedQueryString}" />"> <spring:message
													code="button.export.all.page" />
											</a>
										</c:when>
										<c:otherwise>
											<a href="${pageContext.request.contextPath}/example/crud/emp/exportEmpList"> <spring:message code="button.export.all.page" />
											</a>
										</c:otherwise>
									</c:choose></td>
							</tr>
				...
				<!-- list area -->
				<c:url var="deleteEmpListUrl" value="/example/crud/emp/deleteEmpList" />
				<form:form action="${deleteEmpListUrl}" commandName="integerIdListDto">
					...
					<table class="bodyTable">
						...
						<c:forEach var="empDto" items="${empDtoList}">
							<tr class="b">
								...
								<td><a href="${pageContext.request.contextPath}/example/crud/emp/readEmp?empNo=${empDto.empNo}"> <c:out value="${empDto.empNo}" />
								</a></td>
								<td><a href="${pageContext.request.contextPath}/example/crud/emp/readEmp?empNo=${empDto.empNo}"> <c:out value="${empDto.ename}" />
								</a></td>
					...
					<hr />
					<a href="${pageContext.request.contextPath}/example/crud/emp/createEmp"><spring:message code="button.create" /></a>
