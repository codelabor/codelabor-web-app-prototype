 ...
 *
 */
@ContextConfiguration(locations = { "file:src/main/resources/spring/applicationContext-messages.xml" })
...
public class ReloadableResourceBundleMessageSourceTest {
	...
	private final static Logger logger = LoggerFactory.getLogger(ReloadableResourceBundleMessageSourceTest.class);
	...
	public final void testGetMessageStringObjectArrayLocale() {
		...
		String message = messageSource.getMessage("no.such.message", null, "An unknown error occrred", Locale.ENGLISH);
		logger.debug(message);
		Assert.assertEquals("An unknown error occrred", message);

		message = messageSource.getMessage("no.such.message", null, "알 수 없는 에러가 발생하였습니다.", Locale.KOREAN);
		logger.debug(message);
	...
	public final void testGetMessageStringObjectArrayStringLocale() {
		...
		String message = messageSource.getMessage("button.delete", null, Locale.ENGLISH);
		logger.debug(message);
		Assert.assertEquals("Delete", message);

		message = messageSource.getMessage("button.delete", null, Locale.KOREAN);
		logger.debug(message);
		Assert.assertEquals("삭제", message);
	}
